...
void render_mesh(EngineContext* ctx, mesh* mesh_to_render, vec4* face_color){
  ...
  for(int i = 0; i < mesh_to_render->numTriangles; i++){
    ...
    // world 
    for(int v = 0; v < 3; v++) tri_world.p[v] = vec4_transform(&tri.p[v], &gctx->mat_world);
...
void game_update(EngineContext* ctx){
  ...
  mat4 matRotY    = mat4_rotY(gctx->theta_y); 
  ...
  mat4 mat_trans = mat4_translation(cube->pos.x, cube->pos.y, 5);  
  ...
  else if (currentKeyStates[SDL_SCANCODE_D]) gctx->theta_x += 5*dt; 
  else if (currentKeyStates[SDL_SCANCODE_W]) gctx->theta_z -= 5*dt;
  else if (currentKeyStates[SDL_SCANCODE_S]) gctx->theta_z += 5*dt; 
  else if (currentKeyStates[SDL_SCANCODE_E]) gctx->theta_y -= 5*dt;
  else if (currentKeyStates[SDL_SCANCODE_R]) gctx->theta_y += 5*dt;  
